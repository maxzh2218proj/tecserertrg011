...
function MaterialsTester(){
    ...
    useEffect(() => {
        ...
        axios.get(`http://192.168.168.209:3001/clientstester/byId/${id}`).then((response) => {
            setClientObject(response.data)
        });

        axios.get(`http://192.168.168.209:3001/goodstester/${id}`).then((response) => {
            setGoods(response.data)
        });

        axios.get(`http://192.168.168.209:3001/materialstester/${id}`).then((response) => {
    ...
    const addMaterial = (event) => {
        ...
        axios
            .post("http://192.168.168.209:3001/materialstester", {
            ...
            .then((response) => {
                if(response.data.error){
                    ...
                    const goodToAdd = {materialname: materialname, code: code, mass: mass, normgood: normgood, normorder: normorder, note: note, pki: pki, account: account, datepay: datepay, supplier: supplier, datearrival: datearrival, datevk: datevk, dateotk: dateotk, sertificate: sertificate, dateclosesert: dateclosesert, dop1: dop1, dop2: dop2, dop3: dop3, dop4: dop4, status: status};
    ...
    const handleFileUpload = async (event) => {
        ...
        await axios
          .post(`http://192.168.168.209:3001/materialstester/${id}`, formData, {
            headers: {
      ...
      const deleteMaterial = (id) => {
        if(window.confirm("ВНИМАНИЕ!!! Вы подтверждаете удаление? ОТМЕНИТЬ НЕВОЗМОЖНО!!!")){
            ...
            axios.delete(`http://192.168.168.209:3001/materialstester/${id}`, {
    ...
    const deleteGood = (id) => {
        axios.delete(`http://192.168.168.209:3001/goodstester/${id}`, {
            headers: {accessToken: localStorage.getItem('accessToken')},
        }).then(() => {
    ...
    const deleteClient = (id) => {
        axios.delete(`http://192.168.168.209:3001/clientstester/${id}`, {
            headers: {accessToken: localStorage.getItem('accessToken')},
        }).then(() => {
            history("/");
        })
    ...
    const handleEdit = (id) => {
        axios.get('http://192.168.168.209:3001/materialstester/'+id)
        .then((response) => {
          ...
          usetStatus(response.data.status);
        })
      ...
      const handleUpdate = () => {
        ...
        axios.patch("http://192.168.168.209:3001/materialstester/"+editId, {id: editId, materialname: umaterialname, code: ucode, mass: umass, normgood: unormgood, normorder: unormorder, note: unote, pki: upki, account: uaccount, datepay: udatepay, supplier: usupplier, datearrival: udatearrival, datevk: udatevk, dateotk: udateotk, sertificate: usertificate, dateclosesert: udateclosesert, dop1: udop1, dop2: udop2, dop3: udop3, dop4: udop4, status: ustatus})
        .then(res => {
          console.log(res)
      ...
      const handleUpdate1 = async () => {
        ...
        await axios.patch(`http://192.168.168.209:3001/materialstester/pkii/${editId}`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          })
      ...
      const handleUpdate2 = async () => {
        ...
        await axios.patch(`http://192.168.168.209:3001/materialstester/sert/${editId}`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          })
      ...
      const deletePki = async (pkiId) => {
        ...
        try{
            await axios.delete(`http://192.168.168.209:3001/materialstester/pkii/${pkiId}`).then(response => {
                alert('Архив удалён');
            })
        }catch(error){
            console.log(error)
      ...
      const deleteSert = async (sertId) => {
        ...
        try{
            await axios.delete(`http://192.168.168.209:3001/materialstester/sert/${sertId}`).then(response => {
                alert('Архив удалён');
            })
        }catch(error){
            console.log(error)
    ...
    return(
        <div className="materialPage">
            <Link to="/clientstester" className="ListCl">Список заказчиков</Link>
            ...
            <div className="rightSide">
                <form action="" initialValues={initialValues} onSubmit={addMaterial}>
                    ...
                    <input name="status" placeholder="Статус" autoComplete="off" value={status} onChange={(event) => {setStatus(event.target.value)}} />
                        ...
                        <tbody>
                            {materials.map((usr, i) => {
                                ...
                                <tr key={i}>
                                    ...
                                    <td>
                                        {usr.code}<br />
                                        <input type="text" value={ucode} onChange={e => usetCode(e.target.value)}/>
                                    </td>
                                    <td>
                                        {usr.mass}<br />
                                    ...
                                    <td>
                                        {usr.dateotk}<br />
                                        <input type="text" value="Недоступно"/>
                                    </td>
                                    <td>
                                        {usr.sertificate}<br />
                                    ...
                                    <td>
                                        {usr.status}<br />
                                        <input type="text" onChange={e => usetStatus(e.target.value)}/>
                                    </td>
                                    <td>
                                ...
                                <tr key={i}>
                                    ...
                                    <td>{usr.normorder}</td>
                                    <td>{usr.note}</td>
                                    <td><a href={"../Texserver/ПКИ/"+usr.pki+".pdf"} target="_blank">{usr.pki}</a></td>
                                    <td>{usr.account}</td>
                                    <td>{usr.datepay}</td>
                                    <td>{usr.supplier}</td>
                                    ...
                                    <td>{usr.datevk}</td>
                                    <td>{usr.dateotk}</td>
                                    <td><a href={"../Texserver/Сертификаты/"+usr.sertificate+".pdf"} target="_blank">{usr.sertificate}</a></td>
...
export default MaterialsTester;