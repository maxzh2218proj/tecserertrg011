...
const { materialstester } = require("../models");
...
router.post("/", async (req, res) => {
  ...
  await materialstester.create(material);
  res.json(material);
})
...
router.patch("/:id", async (req, res) => {
  ...
  try{
      ...
      await materialstester.update(req.body, {
          where: {
              id: req.params.id,
          },
      });
...
router.delete("/pkii/:id", async (req, res) => {
  ...
  const Materials = await materialstester.findOne({
    where:{
      id: req.params.id
    }
  });
  ...
  try{
    ...
    const filepath = `../client/build/Texserver/ПКИ/${Materials.pki}`;
...
router.delete("/sert/:id", async (req, res) => {
  ...
  try{
    ...
    const filepath = `../client/build/Texserver/Сертификаты/${Materials.sertificate}`;
